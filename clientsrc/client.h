...
struct					s_client
{
	...
	int					(*exec_cmd)(struct s_client *const,
							struct s_client_msg const *const);
	int					(*queuemsg)(struct s_client *const, char *msg);
	int					(*sendmsgs)(struct s_client *const);
	int					(*event)(struct s_client *const,
							char const *const event);
	int					(*parse_input)(struct s_client *const,
							struct s_client_msg *msg, char const *input);

	int					(*connect)(struct s_client *const,
							struct s_client_msg const *const);
	int					(*message)(struct s_client *const,
							struct s_client_msg const *const);
	int					(*nick)(struct s_client *const,
							struct s_client_msg const *const);
	int					(*join)(struct s_client *const,
							struct s_client_msg const *const);
	int					(*user)(struct s_client *const,
							struct s_client_msg const *const);
	int					(*leave)(struct s_client *const,
							struct s_client_msg const *const);
	int					(*quit)(struct s_client *const,
							struct s_client_msg const *const);
	int					(*who)(struct s_client *const,
							struct s_client_msg const *const);

	int					(*eventjoin)(struct s_client *const,
							struct s_message const *const);
	int					(*eventprivmsg)(struct s_client *const,
							struct s_message const *const);
	int					(*eventpart)(struct s_client *const,
							struct s_message const *const);
	int					(*eventping)(struct s_client *const,
							struct s_message const *const);
	int					(*eventmotd)(struct s_client *const,
							struct s_message const *const);
...
int						client_event(struct s_client *const self,
